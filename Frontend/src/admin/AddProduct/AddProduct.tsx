...
//         name: '', category: '', price: '', wholesalePrice: '',
//         oldPrice: '', description: '', ingredients: '', status: 'Active',
//         weight: '', minimumOrder: '1', image: null as File | null
//     });
...
//       oldprice: Number(row.oldprice) || 0,
//       weight: Number(row.weight) || 0,
//       minimumOrder: Number(row.minimumOrder) || 1,
//       description: row.description || "",
//       ingredients: row.ingredients || "",
//       status: row.status || "Active",
...
//                 ...formData,
//                 weight: Number(formData.weight),
//                 minimumOrder: Number(formData.minimumOrder) || 1,
//                 wholesaleprice: formData.wholesalePrice,
//                 oldprice: formData.oldPrice,
//                 productimage: cloudRes.data.secure_url
//             };
...
//             if (res.data.success) {
//                 setMessage({ type: 'success', text: 'Product added to Rasi Bakery inventory!' });
//                 setFormData({ name: '', category: '', price: '', wholesalePrice: '', oldPrice: '', description: '', ingredients: '', status: 'Active', weight: '', minimumOrder: '1', image: null });
//                 setPreview(null);
//             }
//         } catch (err: any) {
//             setMessage({ type: 'error', text: err.response?.data?.message || 'Server Error' });
...
const handleExcelImport = async (
  ...
  try {
    ...
    const formattedProducts = excelRows.map((row) => ({
      ...
      wholesaleprice: Number(row.wholesaleprice) || 0,
      oldprice: Number(row.oldprice) || 0,
      weight: row.weight != null ? String(row.weight) : "", // keep as free text, e.g. "1 piece"
      minimumOrder: Number(row.minimumOrder) || 1,
      description: row.description || "",
      ingredients: row.ingredients || "",
      status: row.status || "Active",
  ...
  text: `${formattedProducts.length} products imported successfully`,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const payload = {
                ...formData,
                weight: formData.weight, // keep as free text, e.g. "1 piece", "2kg" — don't coerce to Number
                minimumOrder: Number(formData.minimumOrder) || 1,
                wholesaleprice: formData.wholesalePrice,
                oldprice: formData.oldPrice,
                productimage: cloudRes.data.secure_url
  ...
  >
    ...
    Import Products
            ...
            <input 
                ...
                onChange={handleChange} 
                required 
                placeholder="e.g. 1 piece, 2kg, 250g" 
            />
        </div>
                            </div>
                            <div className="ap-row">
