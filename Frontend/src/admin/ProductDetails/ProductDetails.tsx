...
//     "Old Price": product.oldprice || "",
//     "Weight (kg)": product.weight ?? "",
//     "Minimum Order Qty": product.minimumOrder ?? 1,
//     "Description": product.description || "",
//     "Status": product.status,
//     "Image URL": product.productimage || ""
//   }));
...
//   category: getCategoryId(editingProduct.category) || null,
//   weight: Number(editingProduct.weight) || 0,
//   minimumOrder: Number(editingProduct.minimumOrder) || 1,
// };
...
//                   <th className="hide-on-mobile">Wholesale</th>
//                   <th>Weight</th>
//                   <th className="hide-on-mobile">Min. Qty</th>
//                   <th>Status</th>
//                   <th>Action</th>
//                 </tr>
//               </thead>
...
interface Product {
  ...
  wholesaleprice: string;
  oldprice?: string;
  weight?: string; // free text, e.g. "1 piece", "2kg", "250g"
  minimumOrder?: number | string;
  description?: string;
  productimage: string;
  status: string;
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (productsRes.data.success) {
        const fetchedProducts = productsRes.data.data.map((product: Product) => ({
          ...product,
          weight: product.weight ?? '', // keep as-is text, e.g. "1 piece"
          minimumOrder: product.minimumOrder ?? 1,
        }));
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...editingProduct,
        category: getCategoryId(editingProduct.category) || null,
        weight: editingProduct.weight ?? '', // keep as free text, don't coerce to Number
        minimumOrder: Number(editingProduct.minimumOrder) || 1,
      };
