...
const role= localStorage.getItem("userRole");
 const RAZORPAY_KEY = import.meta.env.VITE_RAZORPAY_KEY;
  ...
  // ── Helper: read a product's minimum order qty, defaulting to 1 ──
  const getMinOrder = (item: any) => Number(item?.productId?.minimumOrder) || 1;

    const parseWeightNumber = (weightStr: any): number => {
    if (!weightStr) return 0;
    const match = String(weightStr).match(/[\d.]+/);
    return match ? parseFloat(match[0]) : 0;
  };

  // Enforce minimum order on whatever items arrive from cart/buy-now
  const [passedItems, setpassedItems] = useState(() => {
    const items = location.state?.items || [];
    return items.map((item: any) => {
      const minOrder = getMinOrder(item);
      return item.quantity < minOrder ? { ...item, quantity: minOrder } : item;
    });
  });
  ...
  const totalWeight = passedItems.reduce(
  (total: number, item: any) =>
    total + parseWeightNumber(item.productId.weight) * item.quantity,
  0
  );
...
const checkPincodeCharge = async (zipCode: string) => {
  ...
  // ── Quantity update, clamped to each product's minimumOrder ──
  const updateQuantity = (index: number, quantity: number) => {
    const updatedItems = [...passedItems];
    const minOrder = getMinOrder(updatedItems[index]);

    updatedItems[index] = {
      ...updatedItems[index],
      quantity: quantity < minOrder ? minOrder : quantity,
    };
  ...
  const decreaseQuantity = (index: number) => {
    const minOrder = getMinOrder(passedItems[index]);
    const nextQty = passedItems[index].quantity - 1;

    if (nextQty < minOrder) {
      showToast(`⚠️ Minimum order for "${passedItems[index].productId.name}" is ${minOrder}`);
  ...
  // ── Derived: does any item currently violate its minimum order? ──
  const violatingItem = passedItems.find(
    (item: any) => item.quantity < getMinOrder(item)
  );
  const hasMinOrderViolation = !!violatingItem;
  ...
  const handlePlaceOrder = async () => {
    ...
    if (hasMinOrderViolation) {
      const minOrder = getMinOrder(violatingItem);
      return showToast(
        `⚠️ "${violatingItem.productId.name}" requires a minimum order quantity of ${minOrder}`
      );
    }
    ...
    // Backend rejected (e.g. minimum order enforced server-side too)
  ...
  Continue to Payment <ChevronRight size={18} />
              ...
              <motion.div key="payment" initial={{ x: 24, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 24, opacity: 0 }} transition={{ duration: 0.25 }} className="rasi-step-wrapper">
                ...
                {hasMinOrderViolation && (
                  <p className="rasi-min-order-warning">
                    ⚠️ "{violatingItem.productId.name}" needs at least {getMinOrder(violatingItem)} qty. Please update it in the summary before placing the order.
                  </p>
                )}
                ...
                <button
                  ...
                  disabled={isSaving || hasMinOrderViolation}
                >
                  {isSaving ? <Loader2 className="rasi-spinner" size={18} /> : <ShoppingBag size={18} />}
                  {isSaving ? "Placing Order..." : `Place Order • ₹${total}`}
                </button>
        ...
        <aside className="rasi-summary-sticky-card">
          ...
          <div className="rasi-summary-items-list rasi-mini">
            {passedItems.map((item: any, idx: number) => {
              const minOrder = getMinOrder(item);
              const belowMin = item.quantity < minOrder;
              return (
              <div key={idx} className="rasi-summary-item-row">
                <div className="rasi-mini-img-wrap">
                  <img src={item.productId.productimage} alt="" className="rasi-mini-img" />
                ...
                <div className="rasi-item-info">
                  ...
                  <p className="rasi-item-price-each">₹{item.productId.price} × {item.quantity}</p>
                  {minOrder > 1 && (
                    <p className={`rasi-min-order-note ${belowMin ? 'rasi-min-order-note-error' : ''}`}>
