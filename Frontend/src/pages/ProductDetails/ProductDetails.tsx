...
import { motion, AnimatePresence } from 'framer-motion';
import { useCustomerAuth } from '../../context/CustomerAuthContext';
import { ArrowLeft, Weight, PackageCheck } from "lucide-react";
import axios from 'axios';
// import toast from 'react-hot-toast';
import toast, { Toaster } from 'react-hot-toast';
import "./ProductDetails.css"
...
const isRetailer = isLoggedIn && customer?.role?.toLowerCase() === 'retailer';
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      toast.success(
        minQty > 1 ? `Added ${minQty} to Cart (minimum order)` : "Added to Cart!"
      );
    } catch (error) {
      toast.error("Failed to add to cart");
    } finally {
  ...
  <span>Back</span>
      ...
      <div className="rasi-pd-container">
        ...
        <div className="rasi-pd-right">
          ...
          {product.minimumOrder != null && product.minimumOrder > 1 && (
            <div className="rasi-pd-min-order-banner">
              <PackageCheck size={16} />
              <span>Minimum order quantity: <strong>{product.minimumOrder}</strong></span>
            </div>
          )}
            ...
            <h3><Weight size={18} /> Weight</h3>
            <p>{product.weight != null && (
              <span className="rasi-pd-weight-pill">{product.weight}</span>
            )}</p>
          </div>

          <div className="rasi-pd-info-block">
            <h3><PackageCheck size={18} /> Minimum Order</h3>
            <p>
              <span className="rasi-pd-weight-pill">
                {product.minimumOrder ?? 1} {product.minimumOrder && product.minimumOrder > 1 ? 'units' : 'unit'}
              </span>
            </p>
          </div>
